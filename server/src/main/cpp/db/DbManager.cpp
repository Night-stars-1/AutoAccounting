...
Json::Value DbManager::getBillListGroup(int limit) {
    ...
    Json::Value ret;
    sqlite3_stmt *stmt = getStmt(
            "SELECT strftime('%Y-%m-%d', timeStamp + 28800, 'unixepoch') as date, group_concat(id) as ids FROM billInfo WHERE groupId = 0 GROUP BY date ORDER BY date DESC LIMIT ?;"
    );
    sqlite3_bind_int(stmt, 1, limit);
    int rc;
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
...
void DbManager::insertCate(const std::string &id, const std::string &name, const std::string &icon,
    ...
    sqlite3_bind_text(stmt, count + 5, parent.c_str(), -1, SQLITE_STATIC);
...
Json::Value DbManager::getAllCate(const std::string &parent, const std::string &book, int type) {
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        cate["parent"] = reinterpret_cast<const char *>(sqlite3_column_text(stmt, 4));
